...
class GitHubRuntimeReleaseClient {
  ...
  ///
  /// A missing tag, draft release, extra asset, metadata mismatch, or checksum
  /// mismatch is a hard failure.
  Future<ExactRuntimeArtifact> fetchExactArtifact(
    String releaseTag, {
    ...
    final archive = cache == null
        ? await _downloadArchive(assets[kRuntimeArchiveName]!)
        : await cache.getOrStore(
            metadata.archiveSha256,
            () => _downloadArchive(assets[kRuntimeArchiveName]!),
          );
  ...
  Future<Uint8List> _downloadArchive(Uri archiveUrl) async {
    final response = await _get(archiveUrl, accept: 'application/octet-stream');
...
// ---------------------------------------------------------------------------
// Explicit artifact/lock delivery.
//
// These APIs use only exact release tags or local inputs. They never enumerate
// Releases or infer an artifact from a branch name.
...
void verifyLockedRuntimeTree({
  ...
  lock.validate(expectedRepository: expectedRepository);
  if (File(p.join(root.path, 'runtime-release.json')).existsSync()) {
    throw StateError(
...
Future<Directory> extractAndVerifyRuntimeArtifact({
  ...
  if (File(p.join(destination.path, 'runtime-release.json')).existsSync()) {
    throw StateError(
