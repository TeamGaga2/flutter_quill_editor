...
来源：`richtext-divider-clipboard.md`

| ID       | 任务                                         | 优先级     | 备注                                        |
| -------- | -------------------------------------------- | ---------- | ------------------------------------------- |
| `DIV-01` | 复制 HTML / 纯文本放行分隔线（输出 `---\n`） | **已完成** | `rewriteCopyHtml` + `extractPlainText` 调整 |
| `DIV-02` | 粘贴/拖放放行 `<hr>` 与 `stripEmbeds` 白名单 | **已完成** | 放行自产与外源 `<hr>`，纯文本 `---` 不升格  |
| `DIV-03` | 单测全量覆盖与 runtime / client 构建同步     | **已完成** | 测试用例覆盖与产物同步                      |
